...
from cd_ssm import pcn_langevin as pcnl
...
def kernel(
    ...
    def Gamma_t_tilde(x_t_m_1, x_t, params):
        ...
        u_grad_log_pdf = pcnl.logpdf(u_t, aux_u_t, rho_t, u_Gamma_grad, dt)
    ...
    def G_t_tilde(x_t_m_1, x_t, params):
        ...
        u_prop_log_pdf = pcn.logpdf(u_t, aux_u_t, rho_t, dt)
    ...
    # auxiliary weiner proposals
    dt_0, dts = Gamma__0_params[-1], Gamma_params[-1]
    dts = jnp.insert(dts, 0, dt_0, axis=0)

    rhos = jnp.broadcast_to(jnp.atleast_1d(rho), (T,))
    pcn_proposal = jax.vmap(pcn.propose, in_axes=(0, 0, 0, 0, None))
    pcn_langevin_proposal = jax.vmap(pcnl.propose, in_axes=(0, 0, 0, 0, 0, None))
    aux_us = pcn_langevin_proposal(keys_aux_u, u_star, rhos, u_grad_log_w_star, dts, 1)                       # aux_u_t = rho*u_t + sqrt(1 - rho^2) * W_t()
    us = pcn_proposal(keys_proposals_u, aux_us, rhos, dts, N + 1) 
...
# Gamma_val shape is (N+1,), Gamma_u_grad shape is (N+1, m, d_x), u_t shape is (d_x,), x_t shape is (N+1, d_x)
