...
def plot_adaptation_with_d(dirpath, t: int = 10, steps: int = 100, mesh: int = 50):
    ...
    ax[0].plot(ref1, np.log(1 - rhos), label="Log rho")
    ax[0].plot(ref1, - 0.3 * ref1, label="- 1/3 * log D")
    ...
    ax[1].plot(ref1, np.log(deltas), label="Log delta")
    ax[1].plot(ref1, - ref1, label="- log D")
